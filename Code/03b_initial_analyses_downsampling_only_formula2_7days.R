...
share_delisted_by_week <- 
  ...
  group_by(date,delisted_in_next7days) %>% 
  summarise(n = n()) %>% 
  pivot_wider(id_cols = date, 
              names_prefix = "delisted_",
              names_from = delisted_in_next7days, 
              values_from = n) %>% 
...
for (train_end in c("2018-12-31",
  ...
  save(list = "summary_all_algos",
       file = "summary_all_algos_downsampling_formula2_7days.RData")
...
writexl::write_xlsx(summary_all_algos,
                    "summary_all_algos_downsampling_formula2_7days.xlsx")

writexl::write_xlsx(missings_summary_all,
                    "missings_summary_all_formula2_7days.xlsx")
