...
parameters_rf_down <- expand.grid(mtry = seq(5, 120, 5),
                                  splitrule = "gini",
...
for (train_end in c("2018-12-31",
  ...
  # remove tag columns with low frequencies
  tag_columns <- grep("tag_", names(data_train), value = TRUE)
  
  zero_columns <- names(which(colSums(data_train %>% 
                                        dplyr::select(all_of(tag_columns))) < 0.01*nrow(data_train)))
  
  predictors_all2_ <- predictors_all2
  
  if(length(zero_columns) > 0) {
    for (i_col in 1:length(zero_columns)) {
      predictors_all2_ <- predictors_all2_[-which(predictors_all2_ == zero_columns[i_col])]
    }
  }
  
  formula2_delisted_28days_rf <- as.formula(paste0("delisted_in_next28days ~ ",
                                                   paste0(predictors_all2_, collapse = " + ")))
  
  # table(data_train_downsampled$delisted_in_next28days)
  
  # downsampled training data
  
  set.seed(987654321)
  
  data_train_downsampled <- ROSE(delisted_in_next28days ~ .,
                                  data = data_train %>% 
                                    dplyr::select(delisted_in_next28days,
                                                  all_of(predictors_all2_)),
  ...
  for(i_row in 1:nrow(parameters_rf_down)) {
    ...
    model_rf_train_down <- train(formula2_delisted_28days_rf  ,
    ...
    if(!exists("summary_all_algos")) {
      summary_all_algos <- summary_train_rf
    } else {
  ...
  save(list = "summary_all_algos",
       file = "summary_all_algos_rose_formula2_rf.RData")
...
summary_all_algos <- summary_all_algos %>% 
  distinct()

writexl::write_xlsx(summary_all_algos,
                    "summary_all_algos_rose_formula2_rf.xlsx")
